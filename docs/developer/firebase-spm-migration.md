...
| Clean CI build | Local source snapshot, Codemagic simulator command | Locked pub restore and SwiftPM `--config-only` passed without copied project caches. Full Codemagic simulator command passed (`** BUILD SUCCEEDED **` in `clean-ci-build.log`). Both native lockfiles retained identical pins. Hosted CI remains unverified. |
